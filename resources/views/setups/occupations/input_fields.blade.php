<div>
    <div class="mb-4">
        <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Occupation</label>
        <input type="text" id="name" name="name" value="{{ old('name', $occupation->name ?? '') }}"
    ...
    <div class="flex-1">
        ...
        <select id="status" name="status" id="status"
            ...
            @foreach ($status as $key => $value)
                <option value="{{ $key }}"
                    {{ old('status', $occupation?->status ?? '1') == $key ? 'selected' : '' }}>
                    {{ $value }}
                </option>
            @endforeach
        </select>
