<x-app-layout>
    <x-page-layout :title="'Fees List'">

        <x-slot name="actions">
            <a href="{{ route('fee-setup-masters.create') }}"
            ...
            <div class="space-y-6 mb-6">
                ...
                <div class="rounded-xl p-6">
                    ...
                    <div class="bg-white shadow rounded-xl p-6">
                        <form action="{{ route('fee-setup-masters.index') }}" method="GET">
                            @csrf
                    ...
                    <div class="bg-white shadow rounded-xl p-6 mt-6">
                        ...
                        <div class="my-4">
                            <h2 class="text-xl font-semibold text-gray-700">Fee Setup Masters</h2>
                            <p class="text-sm text-gray-500">Below is the list of fee setup masters.</p>
                        </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full text-sm text-left table-auto border">
                                ...
                                <tbody>
                                    ...
                                    dd($fees);
                                @endphp --}}
                                    @foreach ($fee_setup_masters as $key => $feeMasterItem)
                                        <tr class="hover:bg-gray-50">
                                            ...
                                            <td class="px-3 py-2 border text-center">
                                                {{ $feeMasterItem?->feeType?->code }}

                                            </td>


                                            <td class="px-3 py-2 border">
                                                {{ $feeMasterItem?->feeType?->name }},  {{ $feeMasterItem?->feeType?->paymentFrequencyType->display_name }}, {{ $feeMasterItem?->year }}

                                            </td>


                                            <td class="px-3 py-2 border text-center">
                                                {{ $feeMasterItem?->schoolClass?->name }}
                                            </td>

                                            <td class="px-3 py-2 border text-center">
                                                {{ $feeMasterItem?->group?->name }}

                                            </td>

                                            <td class="px-3 py-2 border text-right">

                                                {{ $feeMasterItem?->amount }}

                                            </td>

                                            <td class="px-3 py-2 border text-center">
                                                {{ $feeMasterItem?->paymentFrequencyType?->display_name }}
                        ...
                        <div class="my-4">
                            <h2 class="text-xl font-semibold text-gray-700">Fee Setup Items</h2>
                            <p class="text-sm text-gray-500">Below is the list of fee setup items.</p>
                        </div>
                            ...
                            <table class="min-w-full text-sm text-left table-auto border">
                                ...
                                <tbody>
                                    ...
                                    dd($fees);
                                @endphp --}}
                                    @foreach ($fee_setup_items as $key => $fee)
                                        <tr class="hover:bg-gray-50">
                                            <td class="px-3 py-2 border">
                                                {{ $key + 1 }}
                                            ...
                                            <td class="px-3 py-2 border">
                                                {{ $fee?->feeType?->name }},  {{ date('F', mktime(0, 0, 0, $fee->month, 1)) }} {{ $fee->year }}, {{ $fee->feeType?->paymentFrequencyType?->display_name }}
                                            ...
                                            <td class="px-3 py-2 border text-center">
                                                {{ $fee?->feeType?->paymentFrequencyType->display_name }}
